...
import nus.iss.ADBackend.model.ReportAction;
...
public class adminController {
	...
	public ResponseEntity<JSONObject> getDashboardInformation() {
		...
		int flaggedCount = recipeService.getAllFlaggedRecipes() != null ? recipeService.getAllFlaggedRecipes().size()
				: 0;

		int wpCount = wrongPredictionService.getAllWrongPredictions() != null
				? wrongPredictionService.getAllWrongPredictions().size()
				: 0;
		int pendingWpCount = wrongPredictionService.getPendingPrediction() != null
				? wrongPredictionService.getPendingPrediction().size()
				: 0;
		DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd-MM-yyyy");
		String databaseCreated = userService.findUserByUsername("official-user@gmail.com").getDateCreated()
				.format(formatter);
		int reportCount = reportService.findAllReports() != null ? reportService.findAllReports().size() : 0;
		int pendingReport = reportService.findAllPendingReports() != null ? reportService.findAllPendingReports().size()
				: 0;
