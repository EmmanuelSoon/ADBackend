...
public class CRUDTest {
	...
	public void recipeCreationAndDeleteTest() {
		...
		/*
		 * List<Recipe> rList = rRepo.findAllByDishId(d.getId()); for (Recipe r: rList)
	...
	// public Comment(double rating, String content, User user, LocalDateTime
	// dateTime, Recipe recipe)
	...
	public void CommentCreateAndDeleteTest() {
		...
		// when perform the parent delet operation we shall delete the children first
	...
	private void createIntegredient() {
		// calories per 100g
		iRepo.saveAndFlush(new Ingredient("test-ingredient1", 10.0));
		iRepo.saveAndFlush(new Ingredient("test-ingredient2", 20.0));
	}
	...
	public void WeightIngredientTest() {
		...
		Recipe r1 = createRecipeByUserAndDish(u1, d, t1, "11111");
		createIntegredient();
		r1.getIngredientList().add(new WeightedIngredient(iRepo.findByName("test-ingredient1"), 200));
		r1.getIngredientList().add(new WeightedIngredient(iRepo.findByName("test-ingredient2"), 100));
		...
		Assertions.assertEquals(40.0, got.getTotalCalories());
	...
	public void UserTest() {
		...
		u1.getDislike().add(iRepo.findByName("test-ingredient1"));
		u1.getDislike().add(iRepo.findByName("test-ingredient2"));
	...
	public void UserRewardTest() {
		...
		User u = uService.createUser("test-name", "test-username", "test-password");
		Assertions.assertNotNull(uService.findUserByUserNameAndPassword("test-username", "test-password"));
